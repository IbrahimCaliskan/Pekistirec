...
namespace DataLayer
{
    ...
    public static class IQueryableFreeTextExtensions
    {
        ...
        /// <param name="searchColumnNames">array of full text indexed column names, null or 0 length means use "*"</param>
        /// <param name="useContains">this is a Contains query</param>
        /// <returns></returns>
        static ArrayList ExecuteFreeTextSearch(DbContext dbCtx, string tableName, string primaryKeyColumnName, string searchText, string[] searchColumnNames, string[] selectColumnNames, bool useContains)
        {
            if (String.IsNullOrWhiteSpace(searchText)) throw new ArgumentNullException("searchText");
            string searchColumnNameText = "*";
            if (null != searchColumnNames && searchColumnNames.Length > 0 && searchColumnNames[0].Trim() != "*")
            {
                searchColumnNameText = "" + string.Join(",", searchColumnNames);
            }

            string selectColumnNameText = "*";
            if (null != selectColumnNames && selectColumnNames.Length > 0 && selectColumnNames[0].Trim() != "*")
            {
                selectColumnNameText = "" + string.Join(",", selectColumnNames.Select(cn => "D." + cn).ToArray());
            }
            ...
            string commandType = useContains ? "CONTAINSTABLE" : "FREETEXTTABLE";
            ...
            string sqlQuery = string.Format("SELECT {5} FROM FREETEXTTABLE({1},{3},N'{4}',20) AS CT  INNER JOIN {1} AS D ON CT.[KEY] = D.{0}  ORDER BY CT.[RANK] DESC;", primaryKeyColumnName, tableName, commandType, searchColumnNameText, searchText, selectColumnNameText);
        ...
        /// <param name="searchColumnNames">array of full text indexed column names, null or 0 length means use "*"</param>
        /// <returns></returns>
        static IQueryable<TEntity> FreeTextExInternal<TEntity>(IQueryable<TEntity> thisQuery, DbContext dbCtx, string searchText, string[] searchColumnNames, string[] selectColumnNames,bool useContains) where TEntity : class
        {
            ...
            ArrayList untypedResults = ExecuteFreeTextSearch(dbCtx, tableName, entityKeyNames[0], searchText, searchColumnNames, selectColumnNames, useContains);
        ...
        {
            return FreeTextExInternal<TEntity>(thisQuery, dbCtx, freetextSearchText, null, null,false);
        }
        ...
        /// <returns>IQueryable<TEntity></returns>
        public static IQueryable<TEntity> FreeTextSearch<TEntity>(this IQueryable<TEntity> thisQuery, DbContext dbCtx, string freetextSearchText, string[] searchColumnNames, string[] selectColumnNames) where TEntity : class
        {
            return FreeTextExInternal<TEntity>(thisQuery, dbCtx, freetextSearchText, searchColumnNames, selectColumnNames, false);
        }
        #endregion
        ...
        {
            return FreeTextExInternal<TEntity>(thisQuery, dbCtx, containsSearchText, null, null, true);
        }
        ...
        /// <param name="columnNames">array of full text indexed column names, null or 0 length means use "*"</param>
        /// <returns>IQueryable<TEntity></returns>
        public static IQueryable<TEntity> FreeTextContains<TEntity>(this IQueryable<TEntity> thisQuery, DbContext dbCtx, string containsSearchText, string[] searchColumnNames, string[] selectColumnNames) where TEntity : class
        {
            return FreeTextExInternal<TEntity>(thisQuery, dbCtx, containsSearchText, searchColumnNames, selectColumnNames, true);
        }
        #endregion
    }
}